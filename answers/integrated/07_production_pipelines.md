# Hints and review guidance — Module 07

## Hint 1

A timeout tells you the acknowledgment was not observed. It does not prove the original write failed.
...
Retry the same stable event ID and identical payload. The persisted payload confirms a harmless duplicate. The same ID with different contents is a conflict. Exactly-once economic effects across systems additionally require reconciliation and a source identity contract.

## Debugging guidance

The essential expected evidence is: Stable IDs; restart reconstruction; corruption detection; model/schema contract; documented failure boundaries and rollback; tests of an actual interrupted subprocess in your extension.
